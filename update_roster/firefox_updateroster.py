...
starting_index = 68
for pageindex in range(starting_index, 70):
    print("running script for page index: " + str(pageindex))
    driver.get('https://fantasy.espn.com/basketball/team?leagueId=78370592&teamId=8&seasonId=2020&scoringPeriodId=' + str(pageindex) +'&statSplit=singleScoringPeriod')
    ...
    #     # bench player has game
        if playing == 1:
            ...
            # EXCEPTION CASE: All roster players have games, cannot find swap
            if foundswap == False:
                # unclick current player
                benchbutton.click()
                print("unclicked bench player")

            
    # TODO: Add check for injured players. Choose healthy players over sick players.
    

print("done swapping for all pages")
